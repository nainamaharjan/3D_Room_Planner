...
open class BRYXBanner: UIView {
    ...
    private func addGestureRecognizers() {
        addGestureRecognizer(UITapGestureRecognizer(target: self, action: #selector(BRYXBanner.didTap(_:))))
        let swipe = UISwipeGestureRecognizer(target: self, action: #selector(BRYXBanner.didSwipe(_:)))
        swipe.direction = .up
    ...
    private func initializeSubviews() {
        let views = [ "backgroundView": backgroundView, "contentView": contentView, "imageView": imageView, "labelView": labelView, "titleLabel": titleLabel, "detailLabel": detailLabel ]
    ...
    private func adjustHeightOffset() {
        ...
        if superview === BRYXBanner.topWindow() && self.position == .top {
            let statusBarSize = UIApplication.shared.windows.first?.windowScene?.statusBarManager?.statusBarFrame ?? CGRect.zero
    ...
    open func show(_ view: UIView? = nil, duration: TimeInterval? = nil) {
        let viewToUse = view ?? BRYXBanner.topWindow()
